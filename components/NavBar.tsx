...
const NavLinkWrapper = (props: {text:string,path:string}) => {
  ...
  return <Link href={props.path}>
      <a className={"transition-all border-solid hover:scale-110  duration-125 border-black dark:border-dark-4 focus:border-b-2 motion-reduce:transition-none ease-in-out" + (router.pathname == props.path ? "border-solid border-b-2 border-black  hover:scale-150" : "")}>
     {props.text}
    </a>
    </Link>
